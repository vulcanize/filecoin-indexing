...
func (cs *ChainStore) MessagesForBlock(ctx context.Context, block *types.BlockHeader) ([]types2.TrackedNode, []types2.TrackedNode, error) {
	cst := cbor.NewCborStore(cs.localBlockstore)
	var msgmeta types.MsgMeta
	if err := cst.Get(ctx, block.Messages, &msgmeta); err != nil {
		return nil, nil, xerrors.Errorf("failed to load msgmeta (%s): %w", block.Messages, err)
	}

	blsNodes, err := cs.readAMNodes(msgmeta.BlsMessages)
	if err != nil {
		return nil, nil, xerrors.Errorf("loading bls message nodes for block: %w", err)
	}

	secpkcids, err := cs.readAMTNodes(msgmeta.SecpkMessages)
	if err != nil {
		return nil, nil, xerrors.Errorf("loading secpk message nodes for block: %w", err)
	}
